...
const DatasetInfo: FC<Props> = ({
  ...
  return (
    <div>
      <Link href='/datasets' className={`${expand ? 'w-[80px]' : 'w-[40px] ml-1'} flex justify-center items-center rounded-sm text-[14px] text-gray-500 py-1 mb-3 hover:bg-state-base-hover`}>
      ...
      <div className='pl-1 pt-1'>
        ...
        {expand && <div className='system-xs-regular  my-3 text-text-tertiary first-letter:capitalize'>{description}</div>}
        {extraInfo}
      </div>
    </div>
  )
