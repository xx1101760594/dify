  ...
  <Col>
    ...
    <Properties>
      <Property name='message_id' type='str' key='message_id'>
        For text messages generated by Cubix, simply pass the generated message-id directly. The backend will use the message-id to look up the corresponding content and synthesize the voice information directly. If both message_id and text are provided simultaneously, the message_id is given priority.
      </Property>
...
<Row>
  ...
  <CodeGroup title="Request" tag="GET" label="/meta" targetCode={`curl -X GET '${props.appDetail.api_base_url}/meta' \\\n-H 'Authorization: Bearer {api_key}'`}>
    ...
    {
      "tool_icons": {
        "dalle2": "https://cloud.cubix.ai/console/api/workspaces/current/tool-provider/builtin/dalle/icon",
        "api_tool": {
          "background": "#252525",
          "content": "\ud83d\ude01"
        }
...
<Row>
  ...
  <Col sticky>
    ...
    >
    ```bash {{ title: 'cURL' }}
    curl --location --request POST 'https://api.cubix.ai/v1/apps/annotation-reply/{action}' \
    --header 'Authorization: Bearer {api_key}' \
    --header 'Content-Type: application/json' \
    --data-raw '{
        "score_threshold": 0.9,
